...
/* (begin x1 x2 ... xk) evaluate each expression in order and return the last value */
L f_begin(L t, L e) {
  L r = nil;
  while (!not(t)) {
    r = eval(car(t), e);
...
/* (defun name (params...) body1 [body2 ...])
   Define a function by creating a lambda closure and binding it to name.
   Multiple body forms are wrapped in (begin ...) so the last form is returned. */
L f_defun(L t, L e) {
  L name = car(t);
  L params = car(cdr(t));
  L forms = cdr(cdr(t));
  ...
  if (not(forms)) {
    body = nil; /* empty body => () */
  } else if (not(cdr(forms))) {
    body = car(forms);
  } else {
    body = cons(atom("begin"), forms);
  }
  L clos = closure(params, body, e);
  env = pair(name, clos, env);
  return name;
}
...
/* (macro (name params...) body)
   Create a macro: store name as the pair (macro . closure).
   During application, the evaluator recognizes this wrapper and performs expansion. */
L f_macro(L t, L e) {
  /* t = ((name params...) body) */
  L head = car(t);
  L name = car(head);
  ...
  L body = car(cdr(t));
  L clos = closure(params, body, e);
  /* Tag the closure as a macro with a recognizable wrapper (macro . closure) */
  L wrap = cons(atom("macro"), clos);
  env = pair(name, wrap, env);
  return name;
}
...
L apply(L f, L t, L e) {
  ...
  if (T(f) == CLOS)
    return reduce(f, t, e);
  /* Macro application: f is the pair (macro . clos). */
  if (T(f) == CONS && equ(car(f), atom("macro"))) {
    L clos = cdr(f);                          /* the macro's closure */
    /* Bind raw (unevaluated) arguments to macro params in the macro's closure env */
    L me = bind(car(car(clos)), t, not(cdr(clos)) ? env : cdr(clos));
    /* Evaluate the macro body to produce an expansion (a Lisp form) */
    L expansion = eval(cdr(car(clos)), me);
    /* Evaluate the expansion at the call site (caller environment) */
    return eval(expansion, e);
  }
  return err;
}
